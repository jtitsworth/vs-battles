...
async function fetchVSBattleStats(rosterKey) {
  if (STATS_CACHE[rosterKey]) return STATS_CACHE[rosterKey];
  const r = ROSTER[rosterKey];
  if (!r?.page) return null;
  try {
    const res = await fetch(`/api/fighter?name=${encodeURIComponent(r.page)}`);
    if (!res.ok) return null;
    const data = await res.json();
    if (data.error) return null;
    // Map live VS Battles scores to the str/spd/dur/int/ver shape the engine expects
    const live = {
      str:  Math.round(data.scores.ap),
      spd:  Math.round(data.scores.speed),
      dur:  Math.round(data.scores.durability),
      int:  FIGHTER_STATS[rosterKey]?.int ?? 70,  // intelligence not on VS Battles, keep static
      ver:  Math.min(100, Math.round(data.scores.hax + 20)), // hax score + base versatility
      title:   FIGHTER_STATS[rosterKey]?.title  ?? r.page.replace(/_/g,' '),
      flavor:  FIGHTER_STATS[rosterKey]?.flavor ?? '',
      // Raw text for narrative context
      tier:  data.tier,
      ap:    data.ap,
      speed: data.speed,
      hax:   data.hax,
      liveScore: data.battleScore,
    };
    STATS_CACHE[rosterKey] = live;
    return live;
  } catch(e) {
    return null;
  }
...
function computeBattle1v1(alphaKey, bravoKey, liveA, liveB) {
  const A = liveA || FIGHTER_STATS[alphaKey] || { str:70,spd:70,dur:70,int:70,ver:70, title:"Unknown", flavor:"A mysterious challenger enters the arena." };
  const B = liveB || FIGHTER_STATS[bravoKey] || { str:70,spd:70,dur:70,int:70,ver:70, title:"Unknown", flavor:"A mysterious challenger enters the arena." };
...
function BattleResults({ alpha1v1, bravo1v1 }) {
  ...
  const [liveBravo, setLiveBravo]       = useState(null);
  const [statsLoading, setStatsLoading] = useState(true);
  const [battle, setBattleResult]       = useState(null);
  ...
  // Phase 1: fetch live VS Battles stats for both fighters
  useEffect(() => {
    ...
    Promise.all([
      fetchVSBattleStats(alpha1v1),
      fetchVSBattleStats(bravo1v1),
    ]).then(([a, b]) => {
      // Fall back to hardcoded FIGHTER_STATS if API fails
      const resolvedA = a || FIGHTER_STATS[alpha1v1] || { str:70,spd:70,dur:70,int:70,ver:70,title:'Unknown',flavor:'' };
      const resolvedB = b || FIGHTER_STATS[bravo1v1] || { str:70,spd:70,dur:70,int:70,ver:70,title:'Unknown',flavor:'' };
      setLiveAlpha(resolvedA);
      setLiveBravo(resolvedB);
      setBattleResult(computeBattle1v1(alpha1v1, bravo1v1, resolvedA, resolvedB));
      setStatsLoading(false);
    });
  }, [alpha1v1, bravo1v1]);

  // Phase 2: fetch AI narrative (waits for battle to be computed)
  useEffect(() => {
    if (!battle) return;
    const alphaStats = liveAlpha || FIGHTER_STATS[alpha1v1] || {};
    const bravoStats = liveBravo || FIGHTER_STATS[bravo1v1] || {};
    setLoading(true);
    setError(false);
    setAiRounds(null);
  ...
  return (
    <div>
      {/* Loading state while fetching live VS Battles stats */}
      {statsLoading && (
        <div style={{ textAlign:"center", padding:"48px 0", color:C.muted }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, marginBottom:12 }}>
            <div style={{ width:8,height:8,borderRadius:"50%",background:"#00D6D6",animation:"pulse 1s 0s infinite" }} />
            <div style={{ width:8,height:8,borderRadius:"50%",background:"#00D6D6",animation:"pulse 1s 0.2s infinite" }} />
            <div style={{ width:8,height:8,borderRadius:"50%",background:"#00D6D6",animation:"pulse 1s 0.4s infinite" }} />
          </div>
          <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:12, letterSpacing:"0.15em",
            textTransform:"uppercase", color:"#00D6D6" }}>
            PULLING LIVE STATS FROM VS BATTLES WIKI...
          </div>
          {liveAlpha && !liveBravo && (
            <div style={{ marginTop:8, fontSize:11, color:C.muted }}>✓ {alphaName} — fetching {bravoName}...</div>
          )}
        </div>
      )}
      {!statsLoading && battle && (
      <div>
      {/* Live stats badge */}
      {(liveAlpha?.tier || liveBravo?.tier) && (
        <div style={{ display:"flex", gap:12, marginBottom:12, padding:"8px 12px",
          background:"#00D6D611", border:"1px solid #00D6D633" }}>
          <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:10,
            color:"#00D6D6", letterSpacing:"0.15em", textTransform:"uppercase" }}>
            ✦ LIVE VS BATTLES STATS
          </span>
          {liveAlpha?.tier && <span style={{ fontSize:10, color:C.muted }}>{alphaName}: Tier {liveAlpha.tier}</span>}
          {liveBravo?.tier && <span style={{ fontSize:10, color:C.muted }}>{bravoName}: Tier {liveBravo.tier}</span>}
        </div>
      )}
